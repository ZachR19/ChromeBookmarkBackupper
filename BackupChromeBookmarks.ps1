...
#Change this to wherever you want to backup the bookmarks
$htmlOut = 'C:\temp\ChromeBookmarksBackup.html'
    ...
    Function Get-BookmarkFolder {
        ...
        Process {
            foreach ($child in $node.children) {
                ...
                #date_added - from microseconds (Google Chrome {dates}) to seconds 'standard' epoch.
                $da = [math]::Round([double]$child.date_added / 1000000)

                #date_modified - from microseconds (Google Chrome {dates}) to seconds 'standard' epoch.
                $dm = [math]::Round([double]$child.date_modified / 1000000) 

                if ($child.type -eq 'Folder') {
                    " <dt><H3 add_date=`"${da}`" last_modified=`"${dm}`">$($child.name)</H3>" | Out-File -FilePath $htmlOut -Append -Force -Encoding utf8
